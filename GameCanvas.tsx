...
const GameCanvas: React.FC<GameCanvasProps> = ({ gameState, activeItemId, onAction, swingProgress, isSwimming, velocity }) => {
  ...
  useEffect(() => {
    ...
    // Lazy initialization of the terrain buffer
    ...
    const render = () => {
      ...
      if (terrainBufferRef.current && terrainBufferRef.current.width !== bW) { 
          terrainBufferRef.current.width = bW; 
          terrainBufferRef.current.height = bH; 
      }
